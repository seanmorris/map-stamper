...
var _MapList = require("./MapList");

var _PoolView = require("./PoolView");
...
var Application = /*#__PURE__*/_createClass(function Application() {
  ...
  this.grids = new Map();
  this.stamps = new _Bag.Bag();
  this.pool = new _PoolView.PoolView({
    stamps: this.stamps
  }, this.grids);
  this.maps = new _MapList.MapList({}, this.grids);
  this.stamps.addEventListener('added', function (event) {
    var item = event.detail.item;

    _this.grids.set(item.grid, item);
  });
  this.stamps.addEventListener('removed', function (event) {
    var item = event.detail.item;

    _this.grids["delete"](item.grid);
  });
});
...
;require.register("MapList.js", function(exports, require, module) {
"use strict";
...
var MapList = /*#__PURE__*/function (_View) {
  _inherits(MapList, _View);

  var _super = _createSuper(MapList);

  function MapList() {
    var _this;

    var args = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var grids = arguments.length > 1 ? arguments[1] : undefined;

    _classCallCheck(this, MapList);

    _this = _super.call(this, args);

    _defineProperty(_assertThisInitialized(_this), "template", require('./map-list.html'));

    _this.grids = grids;
    _this.args.maps = new _Bag.Bag();

    _this.listen(_this.args.maps, 'added', function (event) {
      var map = event.detail.item;
      map.addEventListener('activate', function (event) {
        return _this.switchTo(event, event.detail.item);
      });

      _this.switchTo(event, map);
    });
  ...
  _createClass(MapList, [{
    ...
    value: function change(event) {
      ...
      reader.onload = function (event) {
        return _this2.args.maps.add(new _MapView.MapView({
          map: event.target.result
        }, _this2.grids));
      };
    ...
    key: "hideAll",
    value: function hideAll(skip) {
      var _iterator = _createForOfIteratorHelper(this.args.maps.items()),
          _step;
    ...
    key: "switchTo",
    value: function switchTo(event, map) {
      this.hideAll(map);
      this.onTimeout(100, function () {
        return map.args.visible = true;
      });
    }
  }]);
...
;require.register("MapView.js", function(exports, require, module) {
"use strict";
...
var MapView = /*#__PURE__*/function (_View) {
  ...
  function MapView() {
    var _this$args$rotation, _this$args$scale, _this$args$visible;
    ...
    var stamps = args.tags;
    _this = _super.call(this, args);

    _defineProperty(_assertThisInitialized(_this), "template", require('./map.html'));

    _this.grids = grids;
    _this.args.map = _this.args.map;
    _this.args.rotation = (_this$args$rotation = _this.args.rotation) !== null && _this$args$rotation !== void 0 ? _this$args$rotation : 0;
    _this.args.scale = (_this$args$scale = _this.args.scale) !== null && _this$args$scale !== void 0 ? _this$args$scale : 1;
    _this.args.tags = new _Bag.Bag();
    _this.args.width = 773;
    _this.args.width = 773;
    _this.args.visible = (_this$args$visible = _this.args.visible) !== null && _this$args$visible !== void 0 ? _this$args$visible : true;
    var elicitor = new _Elicit.Elicit(_this.args.map);
    elicitor.dataUri().then(function (uri) {
      _this.args.map = uri;
      var image = new Image();

      image.onload = function (event) {
        _this.args.width = image.width;
        _this.args.height = image.height;
      };
    ...
    if (stamps) {
      var _iterator = _createForOfIteratorHelper(stamps),
          _step;

      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var stamp = _step.value;

          _this.args.tags.add(stamp);
        }
      } catch (err) {
        _iterator.e(err);
      } finally {
    ...
    _this.args.tags.addEventListener('added', function (event) {
      var item = event.detail.item;
      ...
      item.used = true;
      item.map = _assertThisInitialized(_this);
    });

    _this.args.tags.addEventListener('removed', function (event) {
      event.detail.item.used = false;
      event.detail.item.map = null;
    });

    _this.crosshairs = new Image();
    _this.crosshairs.src = '/crosshairs.png';
    return _this;
  }

  _createClass(MapView, [{
    key: "dragstart",
    value: function dragstart(event, stamp) {
    ...
    key: "activate",
    value: function activate(event, stamp) {
      var _iterator2 = _createForOfIteratorHelper(this.grids),
          _step2;
    ...
    value: function drop(event) {
      var _stamp$r, _stamp$c;

      event.preventDefault();
      var mapRect = this.tags.map.node.getBoundingClientRect();
      var name = event.dataTransfer.getData("text/plain");
      var stamp = this.grids.get(name);
      stamp.l = (event.clientX + -mapRect.x + -15) / mapRect.width;
      stamp.t = (event.clientY + -mapRect.y + -15) / mapRect.height;
      stamp.r = (_stamp$r = stamp.r) !== null && _stamp$r !== void 0 ? _stamp$r : 0;
      stamp.c = (_stamp$c = stamp.c) !== null && _stamp$c !== void 0 ? _stamp$c : '#00FF00';
    ...
    key: "resetRotation",
    value: function resetRotation(event) {
      this.args.rotation = 0;
    }
  }, {
    key: "resetScale",
    value: function resetScale(event) {
      this.args.scale = 1;
    }
  }, {
    key: "renderImage",
    value: function renderImage() {
      ...
      this.args.rendered = '';
      var svg = this.tags.svg.cloneNode(true);
      ...
      try {
        for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
          ...
          var _iterator4 = _createForOfIteratorHelper(sheet.cssRules),
              _step4;
      ...
      var image = this.tags.image.node;
      return new Promise(function (accept) {
        image.addEventListener('load', function (event) {
          var canvas = _this2.tags.canvas;
    ...
    value: function viewImage() {
      this.renderImage().then(function (blob) {
      ...
      this.renderImage().then(function (blob) {
        if (globalThis.showSaveFilePicker) {
          globalThis.showSaveFilePicker({
            ...
            return writableStream.write(blob).then(function () {
    ...
    key: "rotateLeft",
    value: function rotateLeft(event, stamp) {
      stamp.r -= 0.25;
    }
  }, {
    key: "rotateRight",
    value: function rotateRight(event, stamp) {
      stamp.r += 0.25;
    }
  }, {
    key: "delete",
    value: function _delete(event, stamp) {
...
;require.register("PoolView.js", function(exports, require, module) {
"use strict";
...
var PoolView = /*#__PURE__*/function (_View) {
  _inherits(PoolView, _View);

  var _super = _createSuper(PoolView);
  ...
  function PoolView() {
    ...
    _classCallCheck(this, PoolView);

    _this = _super.call(this, args);

    _defineProperty(_assertThisInitialized(_this), "template", require('./pool.html'));

    _this.grids = grids;
    _this.args.stamps = _this.args.stamps || new _Bag.Bag();
    _this.crosshairs = new Image();
    _this.crosshairs.src = '/crosshairs.png';
    return _this;
  }

  _createClass(PoolView, [{
    key: "dragstart",
    value: function dragstart(event, stamp) {
      var _stamp$r, _stamp$c;
    ...
    key: "activate",
    value: function activate(event, stamp) {
      var _iterator = _createForOfIteratorHelper(this.grids),
          _step;
    ...
    key: "jump",
    value: function jump(event, stamp) {
      if (stamp.map) {
        var _event = new CustomEvent('activate', {
          detail: {
            item: stamp.map
          }
        });

        stamp.map.dispatchEvent(_event);
      }
    }
  }]);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  setTimeout(function () {
    return [{
      ...
      return app.stamps.add(stamp);
    });
  }, 500);
});
});

require.register("map-list.html", function(exports, require, module) {
module.exports = "<div class = \"map-list\">\n\t<div class = \"tabs\">\n\t\t<div class = \"tabs\" cv-each=\"maps:map:m\">\n\t\t\t<div data-visible = \"[[map.args.visible]]\" class = \"tab\" cv-on = \"click:switchTo(event, map);\">[[m]]</div>\n\t\t</div>\n\t\t<input type = \"file\" cv-on = \"change\">\n\t</div>\n\t<div class = \"maps\" cv-each=\"maps:map\">\n\t\t[[map]]\n\t</div>\n</div>\n"
});

;require.register("map.html", function(exports, require, module) {
module.exports = "<div class = \"map\" cv-attr = \"width;height;data-visible:visible\">\n\t<svg xmlns=\"http://www.w3.org/2000/svg\" cv-attr = \"width;height\" cv-ref = \"svg\">\n\t\t<foreignobject x=\"0\" y=\"0\" width = \"100%\" height = \"100%\">\n\t\t\t<div class = \"map\" xmlns=\"http://www.w3.org/1999/xhtml\" style = \"--width:[[width]];--height:[[height]];\">\n\t\t\t\t<div class = \"map-holder\" style = \"--map:url([[map]]); --rotation:[[rotation]]; --scale:[[scale]];\" cv-on = \"drop;dragover;\" cv-ref = \"map\" xmlns=\"http://www.w3.org/1999/xhtml\">\n\t\t\t\t\t<div cv-each = \"tags:stamp:t\" xmlns=\"http://www.w3.org/1999/xhtml\">\n\t\t\t\t\t\t<div class = \"map-tag\"\n\t\t\t\t\t\t\tdraggable = \"true\"\n\t\t\t\t\t\t\tdata-active = \"[[stamp.active]]\"\n\t\t\t\t\t\t\tcv-on = \"dragstart(event, stamp);click:activate(event, stamp)\"\n\t\t\t\t\t\t\tstyle = \"--x:[[stamp.x]];--y:[[stamp.y]];--l:[[stamp.l]];--t:[[stamp.t]];--r:[[stamp.r]];--c:[[stamp.c]];\" xmlns=\"http://www.w3.org/1999/xhtml\">\n\t\t\t\t\t\t\t<p xmlns=\"http://www.w3.org/1999/xhtml\">[[stamp.id]]</p>\n\t\t\t\t\t\t\t<button data-type = \"close\" xmlns=\"http://www.w3.org/1999/xhtml\" cv-on = \"click:delete(event, stamp)\"></button>\n\t\t\t\t\t\t\t<div class = \"buttons\">\n\t\t\t\t\t\t\t\t<button data-type = \"rotate-left\" xmlns=\"http://www.w3.org/1999/xhtml\" cv-on = \"click:rotateLeft(event, stamp)\"></button>\n\t\t\t\t\t\t\t\t<button data-type = \"rotate-right\" xmlns=\"http://www.w3.org/1999/xhtml\" cv-on = \"click:rotateRight(event, stamp)\"></button>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t</div>\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t</foreignobject>\n\t</svg>\n\t<img cv-ref = \"image\" src = \"[[rendered]]\">\n\t<canvas cv-ref = \"canvas\" width = \"[[width]]\" height = \"[[height]]\"></canvas>\n</div>\n\n<div class = \"map-buttons\" cv-attr = \"data-visible:visible\">\n\t<div>\n\t\t<button cv-on = \"click:viewImage\">view</button>\n\t\t<button cv-on = \"click:saveImage\">save</button>\n\t</div>\n</div>\n\n<div class = \"map-controls\" cv-attr = \"data-visible:visible\">\n\t<label>\n\t\t<span>rotation</span>\n\t\t<input type = \"range\" cv-bind = \"rotation\" min =  \"-0.25\" max = \"0.25\" step = \"0.001\">\n\t\t<input type = \"number\" cv-bind = \"rotation\" min =  \"-0.25\" max = \"0.25\" step = \"0.001\">\n\t\t<button cv-on = \"click:resetRotation\">reset</button>\n\t</label>\n\t<label>\n\t\t<span>scale</span>\n\t\t<input type = \"range\" cv-bind = \"scale\"    min =  \"0\" max = \"3\" step = \"0.001\">\n\t\t<input type = \"number\" cv-bind = \"scale\"    min =  \"0\" max = \"3\" step = \"0.001\">\n\t\t<button cv-on = \"click:resetScale\">reset</button>\n\t</label>\n</div>\n"
});

;require.register("pool.html", function(exports, require, module) {
module.exports = "<div class = \"stamp-pool-list\" cv-each = \"stamps:stamp\">\n\t<div class = \"stamp-pool-item\" draggable = \"true\" cv-on = \"dragstart(event, stamp);click:activate(event, stamp)\" tabindex=\"0\" data-used = \"[[stamp.used]]\" data-active = \"[[stamp.active]]\">\n\t\t<div>\n\t\t\t<p><span class = \"indicator\" style = \"background-color: [[stamp.c]]\">[[stamp.id]]</span></p>\n\t\t\t<p></p>\n\t\t\t<p>[[stamp.grid]]</p>\n\t\t</div>\n\t\t<p data-active = \"only\">\n\t\t\t<input type = \"color\" cv-bind = \"stamp.c\">\n\t\t\t<span cv-if = \"stamp.map\">\n\t\t\t\t<button cv-on = \"click:jump(event, stamp)\">go</button>\n\t\t\t</span>\n\t\t</p>\n\t</div>\n</div>\n"
});
