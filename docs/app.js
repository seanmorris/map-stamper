...
var MapView = /*#__PURE__*/function (_View) {
  ...
  _createClass(MapView, [{
    ...
    value: function saveImage() {
      ...
      fetch('/app.css').then(function (response) {
        return response.text();
      }).then(function (response) {
        var styles = document.createElement('style');
        styles.innerHTML = response;
        svg.prepend(styles);
        _this2.args.rendered = 'data:image/svg+xml;utf8,' + encodeURIComponent(svg.outerHTML);
        var image = _this2.tags.image;
...
document.addEventListener('DOMContentLoaded', function () {
  var grids = new Map();
  var pool = new _PoolView.PoolView();

  var maps = _View.View.from("<div><div cv-each=\"maps:map\">[[map]]</div><input type = \"file\" cv-on = \"change\"></div>");

  maps.args.maps = new _Bag.Bag();
  pool.onTimeout(500, function () {
    return pool.args.stamps.push({
      id: 1,
      grid: 'CONQ001',
      x: 25,
      y: 25,
  ...
  pool.args.stamps.bindTo(function (v, k, t, d, p) {
    return grids.set(v.grid, v);
  });

  maps.change = function (event) {
    console.log(event.target.files[0]);
    var reader = new FileReader();

    reader.onload = function (event) {
      return maps.args.maps.add(new _MapView.MapView({
  ...
  pool.dragstart = function (event, stamp) {
